...
bool valid_isbn10(const std::string &isbn)
{
    ...
    sum = std::accumulate(isbn.begin(), isbn.end() - 1, 0, [&weight](const int total, const char c) {
        if (isdigit(c)) {
...
bool valid_isbn13(const std::string &isbn)
{
    ...
    sum = std::accumulate(isbn.begin(), isbn.end(), 0, [&weight](const int total, const char c) {
        if (isdigit(c)) {
            weight = weight == 3 ? 1 : 3;
            return total + (c - '0') * weight;
        }
